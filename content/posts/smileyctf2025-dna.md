...
## TL;DR
This challenge features a vm that takes your flag, treats it as a vector and multiples it by a matrix of constant values, then compares the resulting vector, if the comparision is correct it prints "CORRECT!"
...
we can see that it's a simple stack based vm, the flag is loaded at 0x280, next thing i did was to make a disasembler for the vm:
...
0x3b90 CALL MARSHAL
```
the first thing i noticed is that it is taking chars of the flag and multiplying them with constants then adds them, this is a dot product (which made me pretty convinced it a linear system, i didn't use this in my first solve tho), after that it is invoking instruction 14, which calls marshal code using a key, in this case the key is a flag char, afrer calling marshal it does some stuff, at this point i got tired of reading the disassembly and decided to make my own emulator so i can debug it:
```python
import sys
import marshal
import sys
...
```
this is because the key (which is a flag char) is wrong, after bruteforcing the keys until i got no error i was left with
```
...
after looking at the comparisions i decided to use z3, by supplying a symbolic flag then adding constraints when the comparisions happen
here is my z3 script:
```python
import sys
import marshal
import sys
...
the reason i used z3 in the first solution is because i couldn't extract all the matrix values from the disassembly (the first part has only 11 rows), after the ctf ended i thought of an idea, by multiplying a matrix with a vector (1, 0, 0 ...) the resulting vector will be the first column of the matrix

![matrix equation](/images/matrix-equation.png)

i used this with the fact that the resulting vector is compared to another vector to extract the columns, another thing is that because some chars of the flag are used to decrypt the marshal code, i hardcoded these keys
...
matrix = np.zeros((49, 49), dtype=int) # matrix of constants
...
after running it i get the flag
